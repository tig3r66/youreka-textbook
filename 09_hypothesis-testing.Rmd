...
Take a the following sleep experiment as an example. In this experiment, people try to memorize as many words as possible. Then, they are split into two groups that either sleeps 4 hours or 8 hours. The next day, people try to recall as many words as possible. The following figure shows the rsults:
...
**Significance levels.** Suppose we set a significance threshold, $\alpha$. If p ≤ $\alpha$, then we can reject the null hypothesis. Otherwise, we do not reject the null hypothesis. Further, if p ≤ $\alpha$, our results are considered "statistically significant."
...
```{r p-memes, out.width="50%", fig.align="center", fig.cap="Figure retrieved from [imgflip](https://imgflip.com/i/vtnlk).", echo=FALSE}
