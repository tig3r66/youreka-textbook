...
1.  They neatly package related variables by maintaining a spreadsheet-like row-ordering. Data frames make it easy to filter rows and columns of interest.
...
2.  Below each column heading, we see `<fct>   <fct>   <int>   <dbl>   <int>   <dbl>`. These refer to the **variable type** of that column.
    ...
    a.  `fct` is short for "factor" (kind of like a categorical variable),
    b.  `int` is short for "integer", and
    c.  `dbl` is short for "double" (a number with decimal places).
...
2.  `head()` displays the first 6 rows of your data frame by default, and
3.  `tail()` shows the last 6 rows.

Play around with these functions in the interactive block below!

<!---LEARNR EX 1-->

<iframe class="interactive" id="ch4-ex1" src="https://y-textbook.shinyapps.io/ch4-ex1/" scrolling="no" frameborder="no">
...
<iframe class="interactive" id="ch4-ex2" src="https://y-textbook.shinyapps.io/ch4-ex2/" scrolling="no" frameborder="no">
...
<iframe class="interactive" id="ch4-ex3" src="https://youreka-text.shinyapps.io/ch4-ex3/" scrolling="no" frameborder="no">
...
**Data frame summary.** Use data frames and the [tidyverse](https://www.tidyverse.org/)! The `tidyverse` provides a special type of data frame called a "tibble" that has nice default printing behavior, among other benefits.
...
dplyr is a package for data manipulation. It is built to be fast, highly expressive, and open-minded about how your data is stored. It is installed as part of the the [`tidyverse`](https://github.com/hadley/tidyverse) meta-package and it is among the packages loaded via `library(tidyverse)`.
...
1.   It is not self-documenting. What is so special about rows 241 through 252?
1.   It is fragile. This line of code will produce different results if someone changes the row order of `gapminder`, e.g. sorts the data earlier in the script.
...
<button class="collapsible">Pure, predictable, pipeable (OPTIONAL)</button>
...
<button class="collapsible">Additional resources and cheatsheets for dplyr</button>
<div class="collapse_content">

1. Package home [on CRAN](http://cran.r-project.org/web/packages/dplyr/index.html)
...
1. Development home [on GitHub](https://github.com/hadley/dplyr).
    
1. [RStudio Data Wrangling cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf), covering dplyr and `tidyr`. Remember you can get to these via *Help \> Cheatsheets.*
    
1. [Excellent slides](https://github.com/tjmahr/MadR_Pipelines) on pipelines and dplyr by TJ Mahr, talk given to the Madison R Users Group.
    
1. Blog post [Hands-on dplyr tutorial for faster data manipulation in R](http://www.dataschool.io/dplyr-tutorial-for-faster-data-manipulation-in-r/) by Data School, that includes a link to an R Markdown document and links to videos.
    
1. [Cheatsheet](https://rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) from R Studio for dplyr.
