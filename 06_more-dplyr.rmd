...
In this chapter, we will introduce more data wrangling tools and consolidate or skills with a comprehensive practice question.
...
The `mutate()` function defines and inserts new variables into a data frame. You can refer to existing variables by name.
...
**arrange().** The `arrange()` function reorders rows in a data frame.

---

Imagine you wanted `my_gap` ordered by year then country, as opposed to by country then year.
...
**rename().** As the function name suggests, `rename()` will rename variables in your data frame.
...
<button class="collapsible">Computing with group-wise summaries (OPTIONAL)</button>
...
<button class="collapsible">Window functions (OPTIONAL)</button>
<div class="collapse_content">
...
<button class="collapsible">Introduction to data visualization (OPTIONAL)</button>
<div class="collapse_content">
